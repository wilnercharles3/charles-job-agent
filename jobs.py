                        ...
                        for t, loc in combos:
                                        try:
                                                            params = {"app_id": ADZUNA_APP_ID, "app_key": ADZUNA_APP_KEY,
                                                                                                "results_per_page": 10, "what": t, "content-type": "application/json"}
                                                            if loc:
                                                                                    params["where"] = loc
                                                                                r = requests.get("https://api.adzuna.com/v1/api/jobs/us/search/1", params=params, timeout=TIMEOUT)
                                                            if r.status_code == 200:
                                                                                    for j in r.json().get("results", []):
                                                                                                                jobs.append({"title": j.get("title",""), "company": j.get("company",{}).get("display_name",""),
                                                                                                                                                         "location": j.get("location",{}).get("display_name",""), "description": j.get("description","")[:800],
    ...
    for cat in cats:
                try:
                                r = requests.get("https://www.themuse.com/api/public/jobs",
                                                                 params={"category": cat, "page": 1, "descending": "true"}, timeout=TIMEOUT)
                                if r.status_code == 200:
                                                    for j in r.json().get("results", []):
                                                                            locs = [l.get("name","") for l in j.get("locations",[])]
                                                                            jobs.append({"title": j.get("name",""), "company": j.get("company",{}).get("name",""),
                                                                                "location": ", ".join(locs), "description": j.get("contents","")[:800],
...
def fetch_remoteok(titles):
        jobs = []
        try:
                    r = requests.get("https://remoteok.com/api", headers={"User-Agent": "job-agent/1.0"}, timeout=TIMEOUT)
                    if r.status_code == 200:
                                    data = r.json()
                                    kws = set()
                                    for t in titles:
                                                                                                ...
                                                                                                for j in data[1:]:
                                                                                                                    ...
                                                                                                                    if any(kw in pos or kw in tags for kw in kws):
                                                                                                                                            jobs.append({"title": j.get("position",""), "company": j.get("company",""),
                                                                                                                                                                                 "location": j.get("location","Remote"), "description": j.get("description","")[:800],
        ...
        if not RAPIDAPI_KEY:
            ...
            try:
                            r = requests.get("https://jsearch.p.rapidapi.com/search",
                                                             ...
                                                             params={"query": query, "page": "1", "num_pages": "1"}, timeout=TIMEOUT)
                            if r.status_code == 200:
                                                for j in r.json().get("data", []) or []:
                                                                        ...
                                                                        jobs.append({"title": j.get("job_title",""), "company": j.get("employer_name",""),
                                                                            "location": loc, "description": j.get("job_description","")[:800],
    ...
    if not SERPAPI_KEY:
        ...
        try:
                        r = requests.get("https://serpapi.com/search.json",
                                                         params={"engine": "google_jobs", "q": query, "api_key": SERPAPI_KEY}, timeout=TIMEOUT)
                        if r.status_code == 200:
                                            for j in r.json().get("jobs_results", []):
                                                                    ...
                                                                    jobs.append({"title": j.get("title",""), "company": j.get("company_name",""),
                                                                        "location": j.get("location",""), "description": j.get("description","")[:800],
                                                                        "url": url, "source": "Google Jobs"})
        except Exception as e:
